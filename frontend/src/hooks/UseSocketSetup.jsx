...
const UseSocketSetup = () => {
  ...
  useEffect(() => {
    ...
    const handleMessageReceived = (message) => {
      console.log("message received from student", message);
      messageDispatch({ type: 'ADD_MESSAGE', payload: message });
      newMessageDispatch({ type: 'RESET' });
    };
  ...
  }, [messageDispatch, newMessageDispatch]);
  ...
  return (
    <>
      {user && user.userType === 'student' && handleLogout()}
      {user && user.userType === 'admin' && <Navigate to='/admin-home-page' />}
    </>
  );
};
